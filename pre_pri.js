...
function updateTable() {
  ...
  function displayAverages() {
    ...
    let avgTurnaroundTimeCell = document.createElement('td');
    let avgWaitingTimeCell = document.createElement('td');
    avgTurnaroundTimeCell.innerText = 'Avg Turnaround Time: ' + avgTurnaroundTime.toFixed(2);
    avgWaitingTimeCell.innerText = 'Avg Waiting Time: ' + avgWaitingTime.toFixed(2);
  
    let table = document.getElementById('processes');
    let row = table.insertRow();
    let emptyCell = row.insertCell();
    emptyCell.colSpan = 4;
    row.appendChild(avgTurnaroundTimeCell);
    row.appendChild(avgWaitingTimeCell);
  }
